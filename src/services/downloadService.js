...
/**
 * 解析 sjurl，判断是否为标准 URL 或分块链接
 */
function parseSjurl(sjurl) {
  const isNormalUrl = /^(https?:\/\/)/i.test(sjurl);
  if (isNormalUrl) {
    return { type: 'normal', url: sjurl };
  }
...
async function fetchBlob(url, index, total, statusRef, debugOutputRef, progressRef) {
  try {
    ...
    if (progressRef) {
      progressRef.value = Math.floor(((index + 1) / total) * 100);
    }
...
async function mergeAndDownload(blobs, filename, statusRef, debugOutputRef) {
  ...
  try {
    ...
    if (typeof downloadUrl !== 'undefined' && downloadUrl) {
      URL.revokeObjectURL(downloadUrl);
      addDebugOutput(`错误发生后释放合并 Blob 的 Object URL (编程猫 OSS): ${downloadUrl}`, debugOutputRef);
    }
  }
}
